...
" <<< "$creds" 2>/dev/null)"
  ...
  # rollup은 분기/연간 전종목 분석이라 daily보다 오래 걸린다 — 마진을 더 크게 잡는다.
  local margin_ms=$(( ${TOKEN_MARGIN_MIN:-60} * 60 * 1000 ))

  if [ "$expires_at" -gt "$(( now_ms + margin_ms ))" ] 2>/dev/null; then
    echo "[wrapper] OAuth 토큰 유효 (expires in $(( (expires_at - now_ms) / 60000 ))분)" >&2
    export CLAUDE_CODE_OAUTH_TOKEN="$token"
  else
    echo "[wrapper] OAuth 토큰 잔여 부족(<${TOKEN_MARGIN_MIN:-60}분) — claude CLI로 갱신 시도" >&2
