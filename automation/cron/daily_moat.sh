...
" <<< "$creds" 2>/dev/null)"
  ...
  # "지금 안 만료됐나"가 아니라 "이 작업이 끝날 때까지 버티나"를 본다.
  # daily는 12종목 ~13분이 걸리므로 잔여가 마진보다 적으면 선제 갱신한다.
  # (2026-08-22 사고: 잔여 16분 토큰으로 시작해 마지막 종목에서 401)
...
"$PYTHON" automation/src/daily_moat.py
