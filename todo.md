...
## M.7 라운드트립 전략 확정 — 청산 알림 + 심화 확률

전략(사용자 확정): **알림마다 고정금액(≈100만원) 매수 → baseline(진입 당시 전월말 종가)
복귀 시 전량 청산.** 상장폐지 위험이 없는 상품이므로 물려도 기다린다(여유자금 운용).

### M.7.1 실증 결과 — 대기가 항상 이긴다

QLD 20.1년, 알림마다 100만원 고정:

| 알림 | 전략 | 체결 | 회복률 | 평균수익 | 총손익 | 연환산 |
|---|---|---|---|---|---|---|
| -7% | 즉시 매수 | 70/70 | 93% | +7.6% | 528만원 | 30% |
| -7% | -9.5% 대기 | 63/70 | 90% | +10.7% | 676만원 | 37% |
| -7% | **-12% 대기** | 61/70 | 90% | +13.6% | **828만원** | **47%** |
| -12% | 즉시 | 47/47 | 87% | +11.2% | 529만원 | 30% |
| -12% | -14% 대기 | 43/47 | 84% | +14.4% | 620만원 | 37% |
| -16% | 즉시 | 24/24 | 75% | +13.2% | 317만원 | 22% |
| -16% | -18.5% 대기 | 23/24 | 74% | +17.2% | 395만원 | 25% |

기다리다 못 사는 기회비용(7/70)을 손실로 계산해도 대기가 압승. 이유는 심화 확률:
**-7% 알림 후 -9.5% 도달 81%, 추가 하락 없이 회복은 3%뿐.** -7%는 바닥이 아니라 통과 지점.

### M.7.2 결합 심화 확률 (메시지에 실을 것)

**marginal을 여러 개 나열하지 않는다** — 확률 3개를 던지면 종합 판단을 사용자에게 미루는 셈이다.
지표 충족 개수(스코어)로 묶어 **단일 확률**을 낸다. 세 지표는 서로 상관 0.38~0.51로
사실상 "하락 초기냐"라는 한 요인을 세 각도로 본 것이라 스코어로 합치는 게 타당하다.

**-7% 알림 (69건, 무조건 81%)** — 지표: ATH -20% 이내 / VIX<25 / MA200 위
...
(2개 92%·3개 90%로 사실상 동일해 통합.) **하락 초기일수록 더 떨어진다.**

**-12% 알림 (46건, 무조건 77%)** — 방향이 뒤집힌다. 지표: MA200 **아래** / 변동성 높음
...
이 단계에선 '베어장 확인'이 심화 신호다.

**-16% 알림**은 24건뿐이라 조건을 가를 표본이 안 된다 → 무조건 확률만 쓴다(`score_keys: []`).

### M.7.3 ATH 진입권 알림 제거

ATH -35/-40/-45% 진입은 **매수 신호로 근거가 없다**:
12개월 후 중앙 -1.8%·승률 50%(-35%만 보면 -26%·33%). 아무 날이나 매수(+37.1%·81%)보다 나쁘다.
진입 후 중앙 -34% 추가 하락. 극단 낙폭은 "싸진 것"이 아니라 "추세가 꺾인 것".
→ 진입권 알림 제거. **손절 -18%만 유지**(실패 시 평균 -39%라 자본보존 장치는 필요).

### M.7.4 트래커는 이 전략 대상 아님

미회복 건까지 포함한 정직한 손익: 트래커 연 3~10%. SPY buy&hold(연 10.9%)보다 못하다.
→ 트래커·커버드콜 알림은 **정보용**. 확률 블록도 붙이지 않는다.
커버드콜은 77건 전부 회복해 실패 표본 0건 — 4년 상승장 산물이라 판단 보류, 데이터부터 쌓는다.

### M.7.5 구현

1. `trigger_rules.yaml`에 `exit_alert`, `deepening`(레벨별 확률·조건·권고) 추가.
   **주의: YAML 1.1이 `yes`/`no`를 불리언으로 파싱** → 조건 키는 `p_yes`/`p_no`를 쓴다.
2. `index_trigger.py`
   - `fetch_indicators()`: MA200 상/하, ATH 대비, 20일 실현변동성 (돌파 종목만 조회).
     `fetch_vix()`: 확률표 있는 종목이 돌파했을 때만 1회.
   - `deepening_lines()`: 확률표 + 관측 지표 + 조건부 확률 + 대기 권고 블록.
     확률표 없는 종목은 빈 리스트 → 기존 메시지 그대로.
   - **청산 알림**: `state.open[iid]`에 `{baseline, levels, since, entry_ref}` 보관.
     baseline은 **진입 시점 것을 보존**한다(달이 바뀌면 baseline이 달라지므로
     월 리셋에서 `open`을 지우면 안 됨). 같은 달 알림은 한 묶음으로 합치고,
     `cur >= baseline`이면 `✅ 청산` 발송 후 묶음 제거.
   - `qld_roundtrip`에서 ATH 진입권 블록 제거, 손절만 유지.
3. `dd_calibration.py deepen` 서브커맨드 — 확률표 재생성용.
